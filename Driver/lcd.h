...
#define LCD_SCL					(1<<PD7)
#define LCD_SDIN				(1<<PD6)
#define LCD_DC			        (1<<PD5)
#define LCD_RES					(1<<PD4)
...
unsigned char lcd_tx(unsigned char tx,unsigned char dc)
{
	...
	if (dc==0x00)LCD_PORT&=~LCD_DC; else LCD_PORT|= LCD_DC;

          for( i = 0; i < 8; i++)
        {
		 LCD_PORT&=~LCD_SCL;
		 asm("nop");
         if ((tx&0x80)==0x00) LCD_PORT&=~LCD_SDIN; else LCD_PORT|=LCD_SDIN;
...
unsigned char lcd_init()
{
	 LCD_DDR|=LCD_SCL| LCD_SDIN|LCD_DC|LCD_RES;
	  LCD_PORT=0x00;
	  LCD_PORT&=~LCD_RES;
	  LCD_PORT|=LCD_RES;
...
void lcd_putchar(char f)
{
 ...
 }else
 {
   ...
   for(char i=0;i<5;i++)
    {
		char charater = pgm_read_byte(&(Font[f][i]));
     lcd_tx(charater,1);
    }
   lcd_tx(0x00,1);
}
